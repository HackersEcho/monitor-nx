...
import com.dafang.monitor.nx.event.mapper.FrostMapper;
 ...
 */
@Service
public class FrostServiceImpl implements FrostService {
    @Autowired
    private FrostMapper mapper;

    @Override
    public List<PeriodDays> periodDays(FrostParam params) {
        ...
        // 获取数据
        List<FrostPo> periodList = mapper.periodDatas(params);
        ...
        for (String stationNo : staList) {
            // 得到单个站点的所有数据
            List<FrostPo> singleList = periodList.parallelStream().filter(
                    x -> StringUtils.equals(stationNo, x.getStationNo())).collect(Collectors.toList());
            singleList =  periodListHandle(singleList);
          // 得到该站每一个年份发生的天数
            if (singleList.size()>0){
                FrostPo daily = singleList.get(0);
               ...
               // 得到常年天数并保留一位小数
                ...
                for (int i = startYear;i<=endYear;i++){// 得到查询年份的
                    ...
                    String key = i + "";
                    int year = i;
                    Optional<FrostPo> optional = singleList.stream().filter(x -> x.getYear() == year).findFirst();
    ...
    @Override
    public List<DateStatistic> periodDates(FrostParam params) {
        List<DateStatistic> resList = new ArrayList<>();
        int startYear = Convert.toInt(params.getStartYear());
        int endYear = Convert.toInt(params.getEndYear());
        String[] scales = params.getClimateScale().split("-");// 常年值区间段
        int scaleLen = Convert.toInt(scales[1]) -  Convert.toInt(scales[0]) +1;
        ...
        for (String stationNo : staList) {
           ...
           periodDateHandle(singleList);
            ...
            if (singleList.size()>0){
                ...
                List<Double> rankList  = singleList.stream().filter(x -> x.getYear() >= params.getRankStartYear() && x.getYear() <= params.getRankEndYear())
                        .map(x -> x.getVal()).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
                int maxDays = Convert.toInt(daysList.get(0));
                int minDays = Convert.toInt(daysList.get(daysList.size() -1));
                String mostHistoryEarliest = LocalDateUtils.getMD(minDays);// 历史最早发生日期
                String mostHistoryLaste = LocalDateUtils.getMD(maxDays);
                // 得到常年日期天数和常年值月日
                double perenVal =  singleList.stream().filter(x -> x.getYear() >= Convert.toInt(scales[0])
                        && x.getYear() <= Convert.toInt(scales[1])).mapToDouble(x -> x.getVal()).sum() / scaleLen;
                int perenDays = Convert.toInt(perenVal);
                String perenDate = LocalDateUtils.getMD(perenDays);
                List<Map<String, EventVal>> data = new ArrayList<>();
                for (int i = startYear;i<=endYear;i++){// 得到查询年份的
                    Map<String, EventVal> map = new HashMap<>();
                    String key = i + "";
                    int year = i;
                    Optional<FrostPo> optional = singleList.stream().filter(x -> x.getYear() == year).findFirst();
                    EventVal build = EventVal.builder().build();
                    if (optional.isPresent()){
                        double liveVal = optional.get().getVal();
                        String liveDate = optional.get().getMd();
                        double anomalyVal = Convert.toInt(liveVal - perenDays);
                        int rank = rankList.indexOf(liveVal) + 1;
                        build = EventVal.builder().liveVal(liveDate).perenVal(perenDate).anomalyVal(anomalyVal).rank(rank).build();
                    }
                    map.put(key, build);
                    data.add(map);
                }
                DateStatistic periodDays = DateStatistic.builder().stationNo(stationNo).stationName(daily.getStationName()).latitude(daily.getLatitude())
                        .longitude(daily.getLongitude()).perenVal(perenVal).mostHistoryEarliest(mostHistoryEarliest).mostHistoryLaste(mostHistoryLaste).data(data).build();
                resList.add(periodDays);
            }
        }
        return resList;
    ...
    @Override
    public List<PeriodStas> periodSta(FrostStaParam params) {
        List<PeriodStas> resList = new ArrayList<>();// 返回值
        int startYear = Convert.toInt(params.getStartYear());
        int endYear = Convert.toInt(params.getEndYear());
        String[] scales = params.getClimateScale().split("-");// 常年值区间段
        int scaleLen = Convert.toInt(scales[1]) -  Convert.toInt(scales[0]) + 1;
        List<FrostPo> periodList = mapper.periodStas(params);
        // 得到要统计的所有区域
        ...
        for (String region : regions) {
            for (RegionStaEnum anEnum : enums) {
                if (StringUtils.equals(anEnum.getRegionId(),region)){
                    ...
                    // 得到该区域下所有的数据
                    List<FrostPo> singleList = periodList.parallelStream().filter(x -> StringUtils.contains(stas, x.getStationNo())).collect(Collectors.toList());
                    // 得到每一年的该区域发生的站数
                    ...
                    for (Integer year : yearList) {
                        ...
                        List<FrostPo> collect = singleList.stream().filter(x -> x.getYear() == temYear).collect(Collectors.toList());
                        int val = 0;
                        if (collect.size()>0){
    ...
    // 霜冻日期转天
    private void periodDateHandle(List<FrostPo> singleList ){
        // 得到每一个日期长度
        List<Integer> yearList = singleList.stream().map(x -> x.getYear()).distinct().collect(Collectors.toList());
        for (Integer year : yearList) {
            int tYear = year;
            Optional<FrostPo> op = singleList.stream().filter(x -> x.getYear() == tYear).findFirst();
            if (op.isPresent()){
                FrostPo f = op.get();
                int numberOfDays = LocalDateUtils.getNumberOfDays(f.getYear() + "" + f.getMd());
                f.setVal(Convert.toDouble(numberOfDays));
            }
        }
    }
    // 根据霜冻的开始和结束日期计算出发发生的天数
   private List<FrostPo> periodListHandle(List<FrostPo> singleList ){
        List<FrostPo> resList = new ArrayList<>();
        // 得到每一个日期长度
        List<Integer> yearList = singleList.stream().map(x -> x.getYear()).distinct().collect(Collectors.toList());
        for (Integer year : yearList) {
            int tYear = year;
            Optional<FrostPo> endOp = singleList.stream().filter(x -> x.getYear() == tYear && x.getTimeType() == 1).findFirst();
            Optional<FrostPo> firstOp = singleList.stream().filter(x ->x.getYear() == tYear && x.getTimeType() == 2).findAny();
            if (firstOp.isPresent() && endOp.isPresent()){
                FrostPo f = firstOp.get();
                FrostPo e = endOp.get();
                Long differDays = LocalDateUtils.getDifferDays(f.getYear() + "" + f.getMd(), e.getYear() + "" + e.getMd());
                f.setVal(Convert.toDouble(differDays));
                resList.add(f);
            }
        }
        return resList;
